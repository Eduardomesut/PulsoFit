...
- **`src/PulsoFit.tsx`** — styling constants and every screen component. There is no router; `App` (in this file) drives a simple state machine via `fase` (`"hero" | "form" | "scan" | "plan" | "recetario" | "cine" | "restaurantes" | "crear"`) and renders one screen at a time. Key pieces inside this file, top to bottom:
  ...
  - Animation helpers: `Aparece` (wraps content, reveals it with a sticker pop + rotation via `IntersectionObserver` when it scrolls into view; used for the long recipe lists in `Recetario`/`Cine` — pass `delay`/`rot` for staggering) and `FotosFlotantes` (hero-only floating polaroids from `POLAROIDS`; each levitates on a CSS loop and reacts to the cursor with parallax + proximity repulsion driven by a `mousemove` + `requestAnimationFrame` loop writing `--px`/`--py`; hidden below 1080px and disabled under `prefers-reduced-motion`). Keyframes/classes live in the global `<style>` in `App`: `flotar`, `.polaroid`/`.polaroid-marco`, `popIn`, plus the existing `fadeUp`/`kenburns`/`scanline`/`marquesina`.
  ...
  - `Cabecera` — shared top bar: navy `Marquesina` ticker strip plus a white sticker-style bar (navy border + hard shadow) with logo left, centered mono uppercase links, contextual `acciones` right, and `MenuLateral` slide-over panel on narrow screens via the "Menú" button. When logged in and a plan is available (current `datos` or `planGuardado`), a "Mi rutina" link appears first (provided by App through `RutinaCtx`, highlighted when `actual="rutina"`) and jumps back to the `Plan` screen from anywhere. Catalog sections are reachable from it on every screen (App remembers the origin screen in `seccionDesde`): `Recetario` (all recipes incl. cine ones, searchable by name/ingredient — accent-insensitive via `normalizar` — with category filter chips), `Cine` (series/movie dishes) and `Restaurantes` (city/area search that embeds a keyless Google Maps iframe — `output=embed`, no API key — with cuisine-type chips preselected from the user's `tipoDieta`). Both build on the shared `CabeceraSeccion` (banner header), `FichaReceta` (expandable card, scene banner when `fotoEscena` exists, optional `onBorrar` delete button) and `DetalleReceta` (ingredients + steps + YouTube link, also used by `Plan`).
...
- **`supabase/schema.sql`** — full Postgres schema/RLS policies for the Supabase project; run manually in the Supabase SQL editor (not migrated automatically). Currently defines `perfiles` (one row per user, auto-created via trigger on signup), `planes` (one saved plan per user, keyed by `user_id`) and `recetas_comunidad` (user-published recipes: public select, insert own rows only, delete own rows or any row if the JWT email equals the admin email — keep that literal in sync with `ADMIN_EMAIL` in `src/logica.ts`). Future phases (per the file's own comments and `SUPABASE_SETUP.md`) will add `registros` (progress tracking) and `amistades` (friends).
