...
- **`src/PulsoFit.tsx`** — the entire application: styling constants, the recipe catalog, calorie/macro logic, and every screen component. There is no router; `App` (in this file) drives a simple state machine via `fase` (`"hero" | "form" | "scan" | "plan" | "recetario" | "cine"`) and renders one screen at a time. Key pieces inside this file, top to bottom:
  ...
  - Catalog sections reachable from every header via `NavSecciones` (App remembers the origin screen in `seccionDesde`): `Recetario` (all recipes incl. cine ones, searchable by name/ingredient — accent-insensitive via `normalizar` — with category filter chips) and `Cine` (series/movie dishes). Both build on the shared `CabeceraSeccion` (banner header), `FichaReceta` (expandable card, scene banner when `fotoEscena` exists) and `DetalleReceta` (ingredients + steps + YouTube link, also used by `Plan`).
